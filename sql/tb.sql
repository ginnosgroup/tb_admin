...
CREATE TABLE `tb_admin_user` (
  ...
  `ap_list` varchar(128) DEFAULT NULL COMMENT '权限列表 (KJ:会计,GW:顾问,MA:Mara,AD:管理员,TGAD:团购管理员;如果为NULL则为超级管理员,显示所有选项)',
  `adviser_id` int DEFAULT NULL COMMENT '所属顾问编号 (对应tb_adviser.id)',
...
CREATE TABLE `b_service_order_review` (
  ...
  `state` varchar(8) NOT NULL COMMENT '状态 (WAIT:待提交审核,REVIEW:审核中,APPLY:服务申请中,COMPLETE:服务申请完成,FINISH:完成-支付成功,CLOSE:关闭)',
  `type` varchar(8) NOT NULL COMMENT '状态 (APPROVAL:通过,REFUSE:驳回)',
