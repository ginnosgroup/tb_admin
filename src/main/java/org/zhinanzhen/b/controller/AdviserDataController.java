	...
	public void down(@RequestParam(value = "adviserId", required = false) Integer adviserId, HttpServletRequest request,
		...
		try {
			...
			for (AdviserServiceOrderDTO so : serviceOrderList) {
				...
				sheet0.addCell(new Label(7, i, so.getReceiveTypeName(), cellFormat));
				sheet0.addCell(new Label(8, i, so.getServiceOrderReceiveDate(), cellFormat));
				sheet0.addCell(new Label(9, i, so.getServiceCode(), cellFormat));
				sheet0.addCell(new Label(10, i, so.getInstitutionTradingName(), cellFormat));
				sheet0.addCell(new Label(11, i, so.getServiceOrderState(), cellFormat));
				sheet0.addCell(new Label(12, i, so.getIsSettle(), cellFormat));
				sheet0.addCell(new Label(13, i, so.getPerAmount(), cellFormat));
				sheet0.addCell(new Label(14, i, so.getAmount(), cellFormat));
				sheet0.addCell(new Label(15, i, so.getExpectAmount(), cellFormat));
				sheet0.addCell(new Label(16, i, so.getRemarks(), cellFormat));
				i++;
			}
			...
			for (AdviserVisaDTO v : visaList) {
				...
				sheet1.addCell(new Label(7, j, v.getKjApprovalDate(), cellFormat));
				sheet1.addCell(new Label(8, j, v.getReceiveTypeName(), cellFormat));
				sheet1.addCell(new Label(9, j, v.getReceiveDate(), cellFormat));
				sheet1.addCell(new Label(10, j, v.getServiceCode(), cellFormat));
				sheet1.addCell(new Label(11, j, v.getGst(), cellFormat));
				sheet1.addCell(new Label(12, j, v.getBonus(), cellFormat));
				sheet1.addCell(new Label(13, j, v.getBonusDate(), cellFormat));
				sheet1.addCell(new Label(14, j, v.getCommissionState(), cellFormat));
				sheet1.addCell(new Label(15, j, v.getRemarks(), cellFormat));
				j++;
			}
			...
			for (AdviserCommissionOrderDTO c : commissionOrderList) {
				...
				sheet2.addCell(new Label(6, k, c.getKjApprovalDate(), cellFormat));
				sheet2.addCell(new Label(7, k, c.getStudentCode(), cellFormat));
				sheet2.addCell(new Label(8, k, c.getSchoolPaymentAmount(), cellFormat));
				sheet2.addCell(new Label(9, k, c.getSchoolPaymentDate(), cellFormat));
				sheet2.addCell(new Label(10, k, c.getTuitionFee(), cellFormat));
				sheet2.addCell(new Label(11, k, c.getPerTermTuitionFee(), cellFormat));
				sheet2.addCell(new Label(12, k, c.getReceiveTypeName(), cellFormat));
				sheet2.addCell(new Label(13, k, c.getReceiveDate(), cellFormat));
