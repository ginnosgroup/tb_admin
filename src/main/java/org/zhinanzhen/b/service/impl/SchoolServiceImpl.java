...
public class SchoolServiceImpl extends BaseService implements SchoolService {
	...
	private void schoolSetting1(String schoolName, Date startDate, Date endDate, String parameters) {
		...
		list.forEach(co -> {
			...
			LOG.error(co.getId() + "学校设置计算=本次收款金额[" + fee + "]*设置参数[" + parameters + "]*0.01=" + co.getCommission());
			updateGST(co);
			...
			if (_parameter.length == 3) {
				...
				if (list.size() >= min && list.size() <= max) {
					...
					list.forEach(co -> {
						...
						// bs.getCommission());
						LOG.error(co.getId() + "学校设置计算=本次收款金额[" + fee + "]*(设置比例[" + proportion + "]*0.01)+设置金额[" + _fee
								+ "]=" + co.getCommission());
						updateGST(co);
						commissionOrderDao.updateCommissionOrder(co);
					});
			...
			if (_parameter.length == 2) {
				...
				if (list.size() >= number) {
					...
					list.forEach(co -> {
						...
						LOG.error(co.getId() + "学校设置计算=本次收款金额[" + fee + "]*(设置比例[" + proportion + "]*0.01)="
								+ co.getCommission());
						updateGST(co);
						commissionOrderDao.updateCommissionOrder(co);
					});
					...
					list.forEach(co -> {
						...
						if (subjectSettingDo != null)
							if (subjectSettingDo.getPrice() > _fee)
								LOG.error(co.getId() + "学校设置计算=设置金额[" + subjectSettingDo.getPrice() + "]-参数金额[" + _fee
										+ "]=" + co.getCommission());
							else
								LOG.error(co.getId() + "学校设置计算=0.00=" + co.getCommission());
						else if (co.getAmount() > _fee)
							LOG.error(co.getId() + "学校设置计算=本次收款金额[" + co.getAmount() + "]-参数金额[" + _fee + "]="
									+ co.getCommission());
						else
							LOG.error(co.getId() + "学校设置计算=0.00=" + co.getCommission());
						updateGST(co);
	...
	private void schoolSetting7(SchoolSettingDO schoolSetting, Date startDate, Date endDate) {
		...
		list.forEach(co -> {
			...
			// 打印日志
			if (subjectSettingDo != null)
				LOG.error(co.getId() + "学校设置计算=学校设置金额[" + subjectSettingDo.getPrice() + "]=" + co.getCommission());
			else
				LOG.error(co.getId() + "学校设置计算=本次收款金额[" + co.getAmount() + "]=" + co.getCommission());
			updateGST(co);
			commissionOrderDao.updateCommissionOrder(co);
		});
	}
	...
	private void updateGST(CommissionOrderListDO co) {
		...
		if (subagencyDo != null)
			LOG.error(co.getId() + "预收业绩=学校设置计算金额[" + co.getCommission() + "]*subagencyRate["
					+ subagencyDo.getCommissionRate() + "]*1.1=" + co.getExpectAmount());
		else
			LOG.error(co.getId() + "预收业绩=学校设置计算金额[" + co.getCommission() + "]*1.1=" + co.getExpectAmount());
		co.setGst(co.getExpectAmount() / 11);
		LOG.error(co.getId() + "GST=预收业绩[" + co.getExpectAmount() + "]/11=" + co.getExpectAmount());
		co.setDeductGst(co.getExpectAmount() - co.getGst());
		LOG.error(co.getId() + "DeductGST=预收业绩[" + co.getExpectAmount() + "]-GST[" + co.getGst() + "]="
				+ co.getDeductGst());
		if (!CommissionStateEnum.YJY.toString().equalsIgnoreCase(co.getCommissionState())) {
			co.setBonus(co.getDeductGst() * 0.1);
			LOG.error(co.getId() + "月奖=DeductGST[" + co.getDeductGst() + "]*1.1=" + co.getBonus());
		}
	}
