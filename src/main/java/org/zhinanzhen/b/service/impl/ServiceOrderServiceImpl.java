...
public class ServiceOrderServiceImpl extends BaseService implements ServiceOrderService {
	...
	@Override
	public int countServiceOrder(String type, List<String> stateList, String reviewState, int userId, int maraId, int adviserId,
			int officialId) throws ServiceException {
		return serviceOrderDao.countServiceOrder(type, stateList, reviewState, userId > 0 ? userId : null,
				maraId > 0 ? maraId : null, adviserId > 0 ? adviserId : null, officialId > 0 ? officialId : null);
	}

	@Override
	public List<ServiceOrderDTO> listServiceOrder(String type, List<String> stateList, String reviewState, int userId,
			int maraId, int adviserId, int officialId, int pageNum, int pageSize) throws ServiceException {
		...
		try {
			serviceOrderDoList = serviceOrderDao.listServiceOrder(type, stateList, reviewState,
